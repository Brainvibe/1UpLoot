...
def login():
    ...
    if login_user:
        ...
        # if the user doesn't exist return the same error message, so a possible attacker doesn't know if the user really exists or not in db
        error = 'Invalid username/password'
        return render_template('login.html', error_login=error)
...
def register():
    if request.method == 'POST':
        ...
        if existing_user is None:
            # creates a hash for the password user typed
...
# user session that enables permission to edit, add, delete, search
@app.route('/<username>')
...
# search route for platform search
@app.route('/get_search/<search>')
def get_search(search):
    # checks if user is logged in
    if 'username' in session:
        user = session['username']
        # search for platform the user chose in db
